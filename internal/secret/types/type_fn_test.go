...
func TestFnType(t *testing.T) {
	assert.Implements(t, (*secret.Type)(nil), new(FnType))
}

func TestFnType_Validate(t *testing.T) {
	tests := []struct {
		name string
		...
		{
			name:    "Empty",
			message: "missing key: " + FieldFnMasterToken,
			violations: []string{
				"missing key: " + FieldFnMasterToken,
			},
		},
		{
			name: "Valid",
