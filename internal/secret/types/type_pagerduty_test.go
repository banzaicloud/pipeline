...
func TestPagerDutyType(t *testing.T) {
	assert.Implements(t, (*secret.Type)(nil), new(PagerDutyType))
}

func TestPagerDutyType_Validate(t *testing.T) {
	tests := []struct {
		name string
		...
		{
			name:    "Empty",
			message: "missing key: " + FieldPagerDutyIntegrationKey,
			violations: []string{
				"missing key: " + FieldPagerDutyIntegrationKey,
			},
		},
		{
			name: "Valid",
			data: map[string]string{
				FieldPagerDutyIntegrationKey: "",
			},
		},
	}
