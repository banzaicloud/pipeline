...
func (op FeatureOperator) installPrometheusPushGateway(
	...
	chartName, chartVersion := getPrometheusPushgatewayChartParams()
...
func (op FeatureOperator) installPrometheusOperator(
	...
	chartName, chartVersion := getPrometheusOperatorChartParams()
...
func (op FeatureOperator) generateGrafanaSecret(
	...
	username := viper.GetString(config.MonitorGrafanaAdminUserName)
...
func (op FeatureOperator) deleteGrafanaSecret(ctx context.Context, clusterID uint) error {
	secretID, err := op.secretStore.GetSecretIDByName(ctx, getGrafanaSecretName(clusterID))
	if err != nil {
		return errors.WrapIf(err, "failed to get Grafana secret")
	}
...
func (op FeatureOperator) deletePrometheusSecret(ctx context.Context, clusterID uint) error {
	secretID, err := op.secretStore.GetSecretIDByName(ctx, getPrometheusSecretName(clusterID))
	if err != nil {
		return errors.WrapIf(err, "failed to get Prometheus secret")
	}
	return op.secretStore.Delete(ctx, secretID)
...
func getPrometheusOperatorChartParams() (name, version string) {
	name = viper.GetString(config.PrometheusOperatorChartKey)
	version = viper.GetString(config.PrometheusOperatorVersionKey)
	return
}

func getPrometheusPushgatewayChartParams() (name, version string) {
	name = viper.GetString(config.PrometheusPushgatewayChartKey)
	version = viper.GetString(config.PrometheusPushgatewayVersionKey)
	return
}
