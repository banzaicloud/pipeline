...
func (c ServiceConversion) Convert(ctx context.Context, instance v1alpha1.ServiceInstance) (integratedservices.IntegratedService, error) {
	if extracter, ok := c.specConversion[instance.Spec.Service]; ok {
		mappedServiceSpec, err := extracter.ConvertSpec(ctx, instance)
		if err != nil {
	...
	return integratedservices.IntegratedService{}, errors.NewWithDetails("spec extractor not found for service",
