...
func (a DeleteRouteTableActivity) Execute(ctx context.Context, input DeleteRouteTableActivityInput) (err error) {
	...
	if err != nil {
		...
		return errors.WrapIfWithDetails(err, "failed to get route table details", keyvals...)
	}
	...
	if err = errors.WrapIfWithDetails(err, "sending request to delete route table failed", keyvals...); err != nil {
	...
	if err = errors.WrapIfWithDetails(err, "waiting for the completion of delete route table operation failed", keyvals...); err != nil {
		if resp := future.Response(); resp != nil && resp.StatusCode == http.StatusNotFound {
			logger.Warn("route table not found")
			return nil
		}
