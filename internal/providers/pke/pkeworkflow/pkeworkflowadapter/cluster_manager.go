...
func (c *Cluster) GetBootstrapCommand(nodePoolName, url string, urlInsecure bool, token string) (string, error) {
	if awscluster, ok := c.CommonCluster.(pkeworkflow.AWSCluster); ok {
		return awscluster.GetBootstrapCommand(nodePoolName, url, urlInsecure, token)
	}
