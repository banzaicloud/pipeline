...
type VspherePKEClusterUpdater struct {
	logger                      logrus.FieldLogger
	paramsPreparer              VspherePKEClusterUpdateParamsPreparer
...
func MakeVspherePKEClusterUpdater(logger logrus.FieldLogger, pipelineExternalURL string, pipelineExternalURLInsecure bool, secrets clusterUpdaterSecretStore, store pke.VspherePKEClusterStore, workflowClient client.Client) VspherePKEClusterUpdater {
	return VspherePKEClusterUpdater{
		logger: logger,
		paramsPreparer: VspherePKEClusterUpdateParamsPreparer{
			logger:  logger,
			secrets: secrets,
...
func (cu VspherePKEClusterUpdater) Update(ctx context.Context, params VspherePKEClusterUpdateParams) error {
	...
	tenantID := sir.GetValue(secrettype.VsphereTenantID)

	conn, err := pkgVsphere.NewCloudConnection(&vsphere.PublicCloud, pkgVsphere.NewCredentials(sir.Values))
	if err = errors.WrapIf(err, "failed to create new Vsphere cloud connection"); err != nil {
		return err
	}
	...
	var labels map[string]map[string]string
	{
		...
		if err != nil {
			return errors.WrapIf(err, "failed to get Vsphere PKE common cluster by ID")
		}
...
func (cu VspherePKEClusterUpdater) handleError(clusterID uint, err error) error {
...
type VspherePKEClusterUpdateParamsPreparer struct {
	logger  logrus.FieldLogger
	secrets interface {
		Get(organizationID uint, secretID string) (*secret.SecretItemResponse, error)
	}
...
func (p VspherePKEClusterUpdateParamsPreparer) Prepare(ctx context.Context, params *VspherePKEClusterUpdateParams) error {
...
type clusterUpdaterNodePoolPreparerDataProvider struct {
	...
	subnetsClient         pkgVsphere.SubnetsClient
	virtualNetworkName    string
	virtualNetworksClient pkgVsphere.VirtualNetworksClient
}
