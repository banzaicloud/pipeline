...
func (_m *MockService) GetProcess(ctx context.Context, org auth.Organization, id string) (pipeline.Process, error) {
	ret := _m.Called(ctx, org, id)

	var r0 pipeline.Process
	if rf, ok := ret.Get(0).(func(context.Context, auth.Organization, string) pipeline.Process); ok {
		r0 = rf(ctx, org, id)
	} else {
