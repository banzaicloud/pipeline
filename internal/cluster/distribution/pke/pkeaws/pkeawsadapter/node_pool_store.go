...
func (s nodePoolStore) CreateNodePool(
	...
	nodePoolModel := &pkeawsmodel.AmazonNodePoolsModel{
		ClusterID:        clusterID,
		CreatedBy:        createdBy,
		Name:             nodePool.Name,
		StackID:          "",
...
func (s nodePoolStore) UpdateNodePoolStackID(
	...
	var pkeAwsCluster pkeawsmodel.PKEAWSClusterModel
	err = s.db.
		Where(pkeawsmodel.PKEAWSClusterModel{ClusterID: clusterID}).
		Preload("NodePools").
	...
	var nodePoolModel *pkeawsmodel.AmazonNodePoolsModel
	...
	if nodePoolStackID != "" { // Note: using CF stack status from now on as long as it exists.
		nodePoolModel.Status = pke.NodePoolStatusEmpty
...
func (s nodePoolStore) UpdateNodePoolStatus(
	...
	var pkeCluster pkeawsmodel.PKEAWSClusterModel
	err = s.db.
		Where(pkeawsmodel.PKEAWSClusterModel{ClusterID: clusterID}).
		Preload("NodePools").
...
func (s nodePoolStore) ListNodePools(
	...
	existingNodePools = make(map[string]pke.ExistingNodePool, len(pkeCluster.NodePools))
