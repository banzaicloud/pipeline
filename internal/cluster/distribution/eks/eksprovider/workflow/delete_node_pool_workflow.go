...
// DeleteNodePoolWorkflowName is the name of the EKS node pool deletion
// workflow.
...
// input-independent components required to delete an EKS node pool.
type DeleteNodePoolWorkflow struct{}

// DeleteNodePoolWorkflowInput defines the input parameters of an EKS node pool
// deletion.
...
func (w DeleteNodePoolWorkflow) Execute(ctx workflow.Context, input DeleteNodePoolWorkflowInput) (err error) {
...

	{
		...
		err := workflow.ExecuteActivity(
			ctx, commonWorkflow.DeleteNodePoolLabelSetActivityName, activityInput,
		).Get(ctx, nil)
		if err != nil {
			return err
		}
