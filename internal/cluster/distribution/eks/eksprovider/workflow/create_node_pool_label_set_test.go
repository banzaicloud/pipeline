...
func TestCreateNodePoolLabelSet(t *testing.T) {
	...
	for _, testCase := range testCases {
		...
		t.Run(testCase.caseDescription, func(t *testing.T) {
			...
			environment.RegisterActivityWithOptions(
				func(ctx context.Context, input clusterworkflow.CreateNodePoolLabelSetActivityInput) error {
					return testCase.expectedError
				},
				activity.RegisterOptions{Name: clusterworkflow.CreateNodePoolLabelSetActivityName},
			)
			...
			environment.ExecuteWorkflow(func(ctx workflow.Context) error {
				...
				actualError = createNodePoolLabelSetFromEKSNodePool(
					ctx,
...
func TestCreateNodePoolLabelSetAsync(t *testing.T) {
	type inputType struct {
		clusterID uint
		nodePool  eks.NewNodePool
	}
	...
	for _, testCase := range testCases {
		...
		t.Run(testCase.caseDescription, func(t *testing.T) {
			...
			environment.ExecuteWorkflow(func(ctx workflow.Context) error {
				...
				actualFuture := createNodePoolLabelSetFromEKSNodePoolAsync(
					ctx,
					testCase.input.clusterID,
					testCase.input.nodePool,
				)
