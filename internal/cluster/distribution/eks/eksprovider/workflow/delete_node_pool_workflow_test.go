...
func (workflowTestSuite *DeleteNodePoolWorkflowTestSuite) SetupTest() {
	...
	setClusterStatusActivity := NewSetClusterStatusActivity(nil)
	workflowTestSuite.environment.RegisterActivityWithOptions(setClusterStatusActivity.Execute, activity.RegisterOptions{Name: SetClusterStatusActivityName})
}
...
func (workflowTestSuite *DeleteNodePoolWorkflowTestSuite) TestDeleteNodePoolWorkflowExecute() {
	...
	mockMethods := func(
		...
		if input.input.ShouldUpdateClusterStatus &&
			mockErrors[SetClusterStatusActivityName] != nil {
			mocks = append(mocks, SetClusterStatusActivityName)
		}
		...
		for mockIndex, mockID := range mocks {
			...
			case awsworkflow.DeleteStackActivityName:
				activityInput := awsworkflow.DeleteStackActivityInput{
					...
					StackName: GenerateNodePoolStackName(
			...
			case SetClusterStatusActivityName:
				...
				if previousMockError != nil &&
					...
					activityInput := SetClusterStatusActivityInput{
						ClusterID:     input.input.ClusterID,
	...
	testCases := []struct {
		...
		},
		{
			...
			mockErrors: map[string]error{
				SetClusterStatusActivityName: errors.New("test error: SetClusterStatus"),
			},
		},
		{
