...
func (a DeleteNamespaceServicesActivity) Execute(ctx context.Context, input DeleteNamespaceServicesActivityInput) error {
	...
	return errors.WrapIff(a.deleter.Delete(ctx, input.OrganizationID, input.ClusterName, k8sConfig, input.Namespace), "failed to delete services in namespace %q", input.Namespace)
}
